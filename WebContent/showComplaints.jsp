...
function viewForm(id){
	window.open("ShowComplaintDoc?id="+id,"View Form","wdth=200,height=200");
}
function approveRequest(id){
	document.getElementById("message").style.display = "inline";
	window.location="ApproveComplaint?id="+id;
}
		...
		<c:if test="${complaints ne null}">
			<table class="table-s">
				<tr>
					...
					<th width=35%>Complaints</th>
					<th width=15% colspan="3" style="text-align: center;">Operations</th>
				</tr>
				<c:forEach items="${complaints}" var="form">
					<tr>
						<td>${form.getId()}</td>
						<td>${form.getName()}</td>
						<td>${form.getNumber()}</td>
						<td>${form.getMail()}</td>
						<td>${form.getComplaint()}</td>
						<td><button class="btn btn-primary"
								onClick="viewForm('${form.getId()}')">View</button></td>
						<td><button class="btn btn-primary"
								onClick="approveRequest('${form.getId()}')">Approve</button></td>
					</tr>
				</c:forEach>
			</table>
		</c:if>
